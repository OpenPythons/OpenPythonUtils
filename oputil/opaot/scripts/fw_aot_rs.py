...
def no_return_func(func: RawFunction):
...
def build_link(func: RawFunction):
    print("// function:", func)
    print(f"const {conv(func.name)}_: i32 = 0x{hex(func.address)[2:].zfill(8)};")
    print()


def build_val(func: RawFunction):
    print("// object:", func)
    print(f"const {conv(func.name)}: i32 = 0x{hex(func.address)[2:].zfill(8)};")
    print()
...
""".lstrip()
    ...
    for addr, func in symbol_map.items():
        if func.name.startswith("entry_table."):
            ...
            found = True
            print(func)
            mp_execute_bytecode = rtext_dict["mp_execute_bytecode"]  # type: RawFunction
            for i in range(256):
    ...
    for addr, func in sorted(text_map.items()):  # type: int, RawFunction
...
""".strip()
    ...
    folder = Path(r"C:\Users\EcmaXp\Dropbox\Projects\OpenPie\oputil\opaot\scripts\fw_aot_rust")

    def write_cls(name, parent: Optional[str], bfp):
        fname = f"MicroPython_{name}.rs" if name else f"MicroPython.rs"
    ...
    for category, funcs in functions.items():
        ...
        with captrue_stdout() as bfp:
            for addr, func in sorted(funcs.items()):  # type: int, RawFunction
                build_body(func)
